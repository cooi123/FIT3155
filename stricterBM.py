...
def good_suffix_table_pre(p, min_alpha="a", max_alpha="z", verbose=False):
    ...
    for i in range(len(p)-1):
        ...
        prev_char = ord(p[i-1]) - ord(min_alpha)
        if verbose:
            print(
...
def boyer_moore(string, pat, min_alpha=MIN_ALPHA, max_alpha=MAX_ALPHA, verbose=False):
    ...
    while s_i < len(string):
        ...
        # check if the current char match
        if string[s_i] == pat[p_i]:
            ...
            # if not bad char found can shift the remaining pattern length
            bad_char_shift = m if bad_char_val == -1 else p_i - bad_char_val
            ...
            if (p_i == m-1):
                ...
                good_suffix_shift = p_i - good_suffix_val if good_suffix_val != -1 else 1
            ...
            skip_region_end = 0
            # only u match prefix when theres no good suffix
